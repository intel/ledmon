...
=item B<-G> or B<--get-slot> B<--controller-type>=I<controller-type> B<--device>=I<device> B<--print>=I<print>

Command returns a slot specified for the controller type with current state and attached
device name (if any). I<Controller-type> is type of controller (VMD, NPEM, SCSI)
that should be scanned here. I<Device> is devnode of selected drive. I<Print> is
optional parameter, which can be set to retrieve only one specific parameter in
the output (device, slot or state).

=item B<-G> or B<--get-slot> B<--controller-type>=I<controller-type> B<--slot>=I<slot> B<--print>=I<print>

Displays slot details. I<slot> is a unique slot identifier.
