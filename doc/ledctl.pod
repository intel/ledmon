...
Set pattern on given devices and update devices states.
To maintain backward compatibility, ibpi mode is also default for ldctl,
and if no option is given, ibpi mode is executed.
