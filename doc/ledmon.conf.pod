...
The controllers should be separated by comma (B<,>) character. To exclude all
controllers once, path to the controllers directory is enough.
...
To allow all controllers once, path to the controllers directory is enough.
The B<XBD Extended Regular Expressions> are supported.  The deprecated name of
